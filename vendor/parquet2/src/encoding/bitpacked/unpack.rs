...
pub fn unpack32(input: &[u8], output: &mut [u32; 32], num_bits: usize) {
    ...
    for (index, value) in output.iter_mut().enumerate() {
        let start_bit = index * num_bits;
        let start_byte = start_bit / 8;
        let bit_offset = start_bit % 8;
        let byte_count = (bit_offset + num_bits).div_ceil(8);
        let mut window = 0_u64;
        for offset in 0..byte_count {
            window |= u64::from(input[start_byte + offset]) << (offset * 8);
        }
        *value = ((window >> bit_offset) & mask) as u32;
    }
}

mod unpack64 {
    unpack_impl!(u64, 8, 64);
}
...
pub fn unpack64(input: &[u8], output: &mut [u64; 64], num_bits: usize) {
    seq_macro::seq!(i in 0..=64 {
        match num_bits {
            #(i => unpack64::unpack::<i>(input, output),)*
            _ => unreachable!("invalid num_bits {}", num_bits),
        }
    });
}
